...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = emps.Where(e => depts.Any(d => d.DeptNo == e.DeptNo && d.Loc == "CHICAHO")).ToList();
    ...
    public void ShouldCountEmployeesPerDepartment()
    {
        ...
        var result = emps.GroupBy(e => e.DeptNo)
            .Select(p => new {DeptNo = p.Key, Count = p.Count()}); 
    ...
    public void ShouldReturnEmployeesWithCommission()
    {
        ...
        var result = emps.Where(x => x.Comm != null)
    ...
    // SQL: SELECT E.EName, S.Grade FROM Emp E JOIN Salgrade S ON E.Sal BETWEEN S.Losal
    // AND S.Hisal;
    [Fact]
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = emps.GroupBy(e => e.DeptNo).Select( p => new {DeptNo = p.Key, AvgSal = p.Average(e=> e.Sal)}); 
        
        Assert.Contains(result, r => r.DeptNo == 30 && r.AvgSal > 1000);
    }

    // 10. Complex filter with subquery and join
    // SQL: SELECT E.EName FROM Emp E WHERE E.Sal > (SELECT AVG(Sal) FROM Emp
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = emps.GroupJoin(
            ...
            {
                Empl = e,
                AvgSal = d.Average(x => x.Sal)
            }
        ).Where(p => p.Empl.Sal > p.AvgSal)
        .Select(p => p.Empl.EName).ToList();
            ;
